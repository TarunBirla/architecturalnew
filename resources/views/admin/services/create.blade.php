...
<div class="max-w-4xl space-y-8">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-gray-400 mt-1">Configure service category, starting fee, turnaround time, and features.</p>
        </div>
        <a href="{{ route('admin.services') }}" class="text-xs text-gray-400 hover:text-white">&larr; Back to Services</a>
    </div>

    <form action="{{ route('admin.services.store') }}" method="POST" class="bg-brand-card p-8 rounded-2xl border border-brand-border space-y-6">
        @csrf
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="featured_image" required placeholder="https://images.unsplash.com/..."
                   class="w-full px-4 py-2.5 bg-black/60 border border-brand-border rounded-xl text-white text-xs font-mono">
        </div>
